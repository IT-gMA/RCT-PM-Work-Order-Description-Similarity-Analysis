...
JSON_FILE_PATH = '../rcnn_model_trainings/workorder_classifications/pm_harmonisation_classification/saved_permutaions_uid_2.json'
OG_EXCEL_PATH = '../xlsx_resources/for_trainings/maximo_pm_to_gap_pm_desc_map.xlsx'
SAVED_DIR = '../xlsx_resources/for_trainings/harmonised_pm_desc_uid7.2'
...
REQUEST_URL = 'http://prod-01.australiasoutheast.logic.azure.com:443/workflows/0ed70efeabef4468b16ebedd54c62111/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=EwuXI_pguso2LmHjigGYRSOj1z0cNp_oGFUvsrOiGGI'
...
NLP_AUGMENTERS = [
    ...
    # na_word.BackTranslationAug(device='cuda'),
    na_word.SynonymAug(),
    na_word.AntonymAug(),
    ...
    na_word.SpellingAug(),
    na_word.AntonymAug(name='Antonym_Aug', ),
    na_char.KeyboardAug(include_numeric=True),
    na_sen.RandomSentAug(),
    # na_word.WordAugmenter(action='insert'),
]
...
def main():
    ...
    for saved_json_permutaion in tqdm(saved_json_permutaions):
        saved_json_permutaion['main_subjs'] = perform_nlp_aug(saved_json_permutaion['main_subjs'], num_gens=4)
    ...
    for item in tqdm(saved_json_permutaions):
        ...
        for main_subject in set(item['main_subjs']):
            list_of_word_copied = copy.copy(LIST_OF_WORDS)
            util_functions.random_seed_shuffle(seed=int(len(main_subject) * len(item['main_subjs'])), og_list=list_of_word_copied)
            list_of_word_copied = list_of_word_copied[:3]
