...
SAVED_DIR = '../xlsx_resources/for_trainings/harmonised_pm_desc_uid'
...
LIST_OF_WORDS = ["Testing", 'Inspection', "Report"]  # ["Testings", "Cleaning"]
...
def main():
    ...
    LIST_OF_WORDS_PERMUTAION = generate_permutaions(LIST_OF_WORDS)
    ...
    for item in tqdm(saved_json_permutaions):
        ...
        for main_subject in item['main_subjs']:
            for sp_word in LIST_OF_WORDS_PERMUTAION:
                random_frequencies = LIST_OF_MAINTENANCE_FREQS
                util_functions.random_seed_shuffle(seed=item['class_idx'], og_list=random_frequencies)
                random_frequencies = random_frequencies[:math.ceil(len(random_frequencies) / 2)]
        ...
        item['upload_data'] = []
        for possible_term in all_possible_terms if len(all_possible_terms) <= MAX_NUM_SAMPLES else all_possible_terms[0:MAX_NUM_SAMPLES]:
            item['upload_data'].append({'idx': idx, 'class_uid': item['class_uid'], 'harmonised_desc': item['class_label'], 'curr_desc': possible_term})
            idx += 1
